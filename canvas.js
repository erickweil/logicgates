...
class Canvas {
	constructor(width,height, canvasTag) {
		...
		document.addEventListener("touchstart",(e) => {this.touchManager.touchstart(e);}, false);
		document.addEventListener("touchmove",(e) => {this.touchManager.touchmove(e);}, false);
		...
		document.addEventListener("touchcancel",(e) => {this.touchManager.touchcancel(e);}, false);
		document.addEventListener("touchleave",(e) => {this.touchManager.touchleave(e);}, false);
	...
	dozoom(center,amount)
	{
		...
		this.scale *= amount;
		this.scale = Math.max(Math.min(this.scale,20.0),0.1);
...
MyCanvas.setNodes(LogicWorld.nodes);
MyCanvas.setUiNodes(LogicWorld.uinodes);

//MyCanvas.nodes[0].selected = true;
//MyCanvas.nodes[1].selected = true;

//MyCanvas.selectedNodes.push(MyCanvas.nodes[0]);
//MyCanvas.selectedNodes.push(MyCanvas.nodes[1]);
...
document.addEventListener('copy', (e) => {
	...
	var convertedNodes = JSON.stringify(LogicWorld.toSerializable(MyCanvas.selectedNodes));
	
	e.clipboardData.setData('text/plain', convertedNodes);
});
...
document.addEventListener('paste', (e) => {
	...
	var convertedNodes = JSON.parse(paste);
	//alert(paste);
...
var simulationInterval = setInterval(simulateAll, 200);
...
//var redrawInterval = setInterval(redrawAll, 200);